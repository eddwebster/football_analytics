...
Module for visualising Signality's tracking data
...
def plot_pitch( field_dimen = (106.0, 68), field_color ='city-blue', linewidth=2, markersize=20):
    """ plot_pitch
    ...
    if field_color=='city-blue':
        ax.set_facecolor('aliceblue')
        lc = 'midnightblue' # line color
        pc = 'k' # 'spot' colors
    elif field_color=='green':
...
def plot_frame( hometeam, awayteam, figax=None, team_colors=(color_home, color_away), field_dimen = (106.0, 68), include_player_velocities=False, PlayerMarkerSize=12, PlayerAlpha=0.7, annotate=False ):
    ...
    for team,color in zip( [hometeam,awayteam], team_colors) :
        ...
        if annotate:
            [ ax.text( team[x]+0.5, team[y]+0.5, x.split('_')[1], fontsize=12, color=color  ) for x,y in zip(x_columns,y_columns) if not ( np.isnan(team[x]) or np.isnan(team[y]) ) ] 
    # plot ball
    ax.plot( hometeam['ball_x'], hometeam['ball_y'], 'ko', MarkerSize=10, alpha=1.0, LineWidth=0)
    return fig,ax
    
def save_match_clip(hometeam,awayteam, fpath, fname='clip_test', figax=None, frames_per_second=25, team_colors=(color_home,color_away), field_dimen = (106.0, 68), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7):
...
def plot_pitchcontrol_for_event( event_id, events,  tracking_home, tracking_away, PPCF, cmap='RdYlGn', alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (106.0, 68)):
...
def plot_pitchcontrol_for_frame(frame,  tracking_home, tracking_away, PPCF, xgrid, ygrid, alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (106.0,68), team_colors=(color_home, color_away), team_pass='Home'):
...
# Create an array of x and y coordinates of 1 frame in the right format for the Voronoi function and distance to tm and opp calculations
def calc_frame_coordinates(team,frame_number):
    tracking_frame = pd.DataFrame(team.iloc[frame_number])
    tracking_frame = tracking_frame.head(36) # get only x & y coordinates
    tracking_frame = tracking_frame.dropna() # get only players in play
    tracking_frame.columns = ['x']
    tracking_frame.insert(1,'y','')
    tracking_frame = pd.DataFrame(index = [tracking_frame.iloc[::2].index],
                                  data = {'x':tracking_frame['x'].iloc[::2].values, 'y':tracking_frame['x'].iloc[1::2].values}) # split x & y in 2 columns
...
def make_frame(t):
    ...
    PPCF,xgrid,ygrid = generate_pitch_control_for_event(t2+frame_id, tracking_home, tracking_home, tracking_away, params, GK_numbers, field_dimen = (106.0, 68), n_grid_cells_x = 50)
    cmap='YlGn'
    fig, ax = plot_pitchcontrol_for_event(t2+frame_id, tracking_home,  tracking_home, tracking_away, PPCF, cmap, annotate=True )
...
def make_frame_delauney(t):
    ...
    fig, ax = plot_delaunay_for_frame(t2+starting_frame, tracking_home, tracking_away, team='Home', include_player_velocities=True )
...
def plot_EPV_for_event(event_id, events, tracking_home, tracking_away, PPCF, EPV, alpha = 0.7, include_player_velocities=True, annotate=False, autoscale=0.1, contours=False, field_dimen = (106.0, 68)):
    ...
    -----------
        ...
        tracking_home: (entire) tracking DataFrame for the Home team
        tracking_away: (entire) tracking DataFrame for the Away team
        PPCF: Pitch control surface (dimen (n_grid_cells_x,n_grid_cells_y) ) containing pitch control probability for the attcking team (as returned by the generate_pitch_control_for_event in Signality_PitchControl)
        EPV: Expected Possession Value surface. EPV is the probability that a possession will end with a goal given the current location of the ball. 
             The EPV surface is saved in the FoT github repo and can be loaded using Signality_EPV.load_EPV_grid()
        alpha: alpha (transparency) of player markers. Default is 0.7
        include_player_velocities: Boolean variable that determines whether player velocities are also plotted (as quivers). Default is False
        annotate: Boolean variable that determines with player jersey numbers are added to the plot (default is False)
    ...
    if pass_team=='Home':
        cmap = 'Reds'
        lcolor = 'r'
        EPV = np.fliplr(EPV) if sio.find_playing_direction(tracking_home,'Home') == -1 else EPV
    else:
        cmap = 'Blues'
        lcolor = 'b'
        EPV = np.fliplr(EPV) if sio.find_playing_direction(tracking_away,'Away') == -1 else EPV
