...
def plot_pitchcontrol_for_event( frame,  tracking_home, tracking_away, PPCF, xgrid, ygrid, alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (106.0,68), team_pass='Home'):
...
def save_pitch_control_clip_(hometeam,awayteam, fpath, fname='clip_test', figax=None, frames_per_second=25, team_colors=('r','b'), field_dimen = (106.0,68.0), include_player_velocities=True, PlayerMarkerSize=10, PlayerAlpha=0.7):
    """ save_match_clip( hometeam, awayteam, fpath )
    ...
    with writer.saving(fig, fname, 100):
        for i in index:
            ...
            PPCF,xgrid,ygrid = mpc.generate_pitch_control_for_event(i, hometeam, awayteam, params, field_dimen = (106.,68.,), n_grid_cells_x = 50, team_pass='Home')
            
            fig,ax = plot_pitchcontrol_for_event(i,  hometeam, awayteam, PPCF, xgrid, ygrid, annotate=True)
            fig.set_tight_layout(True)
